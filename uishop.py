...
class ShopDialog(ui.ScriptWindow):
	...
	def __SellBulkItems(self):
		# Bulk sell: one aggregate confirmation (total price, item count) instead
		# of a per-item dialog - items already blocked by ANTIFLAG_SELL are
		# silently skipped and counted, not individually popped up.
		snapshot = multiSelectModule.PopDragSnapshot()

		sellList = []
		totalPrice = 0
		for (slotType, slotPos) in snapshot:
			if player.IsEquipmentSlot(slotPos):
				continue
			...
			if player.IsValuableItem(slotPos):
				itemPrice = item.GetISellItemPrice()
				if item.Is1GoldItem():
			...
			sellList.append((slotPos, itemCount))

		skipped = len(snapshot) - len(sellList)
		...
		questionText = "Eladod a kijelolt %d itemet, ossz. %d aranyert?" % (len(sellList), totalPrice)
		if skipped > 0:
			questionText += " (%d item nem eladhato, kimarad)" % skipped

		questionDialog = uiCommon.QuestionDialog()
		questionDialog.SetText(questionText)
		questionDialog.SetAcceptEvent(lambda arg=True: self.RequestBulkSellItem(arg))
		questionDialog.SetCancelEvent(lambda arg=False: self.RequestBulkSellItem(arg))
		questionDialog.Open()
		self.bulkSellList = sellList
		self.questionDialog = questionDialog

		constInfo.SET_ITEM_QUESTION_DIALOG_STATUS(1)

	def RequestBulkSellItem(self, answer):
		if answer:
			slotList = [slotPos for (slotPos, itemCount) in self.bulkSellList]
			if slotList:
				net.SendShopSellPacketBulk(slotList)
				snd.PlaySound("sound/ui/money.wav")
