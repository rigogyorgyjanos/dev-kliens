...
def SetServerData(dayIndex, eventID, eventIndex, startTime, endTime, empireFlag, channelFlag, value0, value1, value2, value3, startRealTime, endRealTime, isAlreadyStart):
	# Show the event on every day it actually spans, not just its start day - dayIndex (the
	# server's single anchor day) is ignored here in favor of parsing the real start/end dates,
	# clipped to the currently displayed month (this calendar only ever shows one month at a time).
	dt = datetime.datetime.today()

	startParts = startTime.split(" ")[0].split("-")
	startYear, startMonth, startDay = int(startParts[0]), int(startParts[1]), int(startParts[2])

	hasRealEnd = endTime != "" and not endTime.startswith("1970-01-01")
	if hasRealEnd:
		endParts = endTime.split(" ")[0].split("-")
		endYear, endMonth, endDay = int(endParts[0]), int(endParts[1]), int(endParts[2])
	else:
		# No scheduled end (EMPIRE_WAR_EVENT/TOURNAMENT_EVENT-style, closed manually by a GM) -
		# keep the original single-day behavior for these instead of guessing an end.
		endYear, endMonth, endDay = startYear, startMonth, startDay

	firstDay = startDay if (startYear == dt.year and startMonth == dt.month) else 1
	lastDay = endDay if (endYear == dt.year and endMonth == dt.month) else 31
	if lastDay < firstDay:
		lastDay = firstDay

	for day in xrange(firstDay, lastDay+1):
		if server_event_data.has_key(day):
			server_event_data[day][eventID] = [day, eventID, eventIndex, startTime, endTime, empireFlag, channelFlag, value0, value1, value2, value3, startRealTime, endRealTime, isAlreadyStart]
		else:
			eventList = {}
			eventList[eventID] = [day, eventID, eventIndex, startTime, endTime, empireFlag, channelFlag, value0, value1, value2, value3, startRealTime, endRealTime, isAlreadyStart]
			server_event_data[day] = eventList
...
class ImageBoxSpecial(ui.ImageBox):
	...
	def OnClickEventIcon(self):
		...
		if GetEventIDData(eventID) != None:
			if GetEventIDData(eventID)[EVENT_IS_ACTIVE] == True:
				...
				if interface:
					if interface.wndEventCalendar:
						interface.wndEventCalendar.OnClick(GetEventIDData(eventID)[EVENT_INDEX])
	...
	def OverInItem(self):
		...
		if interface:
			if interface.wndEventCalendar:
				interface.wndEventCalendar.OverInItem(self.dayIndex)
...
class EventCalendarWindow(ui.BoardWithTitleBar):
	...
	def OnClick(self, eventIndex):
		# Ide johet egyedi kattintas-kezeles esemenytipusonkent, ha kesobb kell.
		pass
