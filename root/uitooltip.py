...
class ToolTip(ui.ThinBoard):
	...
	if app.__BL_SHADER__:
		def OnRender(self):
			shaderMgr.ApplyUIBlur()
			(x, y) = self.GetGlobalPosition()
			shaderMgr.DrawBlurRect(x, y, self.GetWidth(), self.GetHeight())
	...
	if app.ENABLE_RENEWAL_AFFECT_SHOWER:
		def SetSkillEx(self, skillIndex, skillLevel = -1):
			...
			for i in xrange(skill.GetSkillAffectDescriptionCount(skillIndex)):
				self.AppendTextLine(skill.GetSkillAffectDescription(skillIndex, i, skillCurrentPercentage), self.FUCK_YOU)
			self.ResizeToolTip()
