...
class CharacterWindow(ui.ScriptWindow):
	...
	def OverInItem(self, slotNumber):
		...
		if skill.SKILL_TYPE_ACTIVE == skillType:
			...
			# Only 3 real columns exist (see __GetRealSkillSlot); Grand Master/Perfect Master/Sage
			# Master (grade 2/3/4) all display in column 2, whichever is the character's actual grade.
			displaySkillGrade = min(2, skillGrade)

			if overInSkillGrade == displaySkillGrade:
	...
	def __RefreshSkillPage(self, name, slotCount):
		...
		for i in xrange(slotCount+1):
			...
			## ACTIVE
				...
				# Only 3 real columns exist (see __GetRealSkillSlot). Grand Master(2)/Perfect
				# Master(3)/Sage Master(4) all share column 2 - whichever is the character's
				# actual current grade wins there.
				for j in xrange(3):
					realSlotIndex = self.__GetRealSkillSlot(j, slotIndex)
					# Column 2's icon always represents the deepest tier reachable there (Sage Master,
					# once that system is enabled), same as columns 0/1 always show their own fixed icon.
					iconGrade = 3 if (j == 2 and app.ENABLE_SKILLS_LEVEL_OVER_P) else j
					skillPage.SetSkillSlotNew(realSlotIndex, skillIndex, iconGrade, skillLevel)
					skillPage.SetCoverButton(realSlotIndex)

					if (skillGrade >= 3) and j == 2:
	...
	def __GetRealSkillSlot(self, skillGrade, skillSlot):
		# Skill_Active_Slot (CharacterWindow.py) only has 3 real grade columns (0/1/2) hand-listed
		# in its uiscript "slot" tuple - there's no 4th column for Sage Master. Grade values at or
		# past the last real column (Grand Master, Perfect Master, Sage Master) all share column 2,
		# same as Perfect Master already did before Sage Master existed.
		return skillSlot + min(2, skillGrade)*skill.SKILL_GRADE_STEP_COUNT
