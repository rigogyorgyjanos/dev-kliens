		...
		if mouseModule.mouseController.IsAttachedMoney():
			...
			if (player.SLOT_TYPE_INVENTORY == attachedSlotType
				...
				if multiSelectModule.dragSnapshot:
					self.__ProcessBulkTradeAdd(attachedSlotType, SrcSlotNumber, DstSlotNumber)
				else:
					net.SendExchangeItemAddPacket(attachedInvenType, SrcSlotNumber, DstSlotNumber)

		mouseModule.mouseController.DeattachObject()

	def __ProcessBulkTradeAdd(self, srcSlotType, usedSrcSlot, usedDstSlot):
		# The clicked item goes exactly where clicked; the rest of the selection
		# auto-fills the remaining free offer slots (capped at
		# exchange.EXCHANGE_ITEM_MAX_NUM). Whatever doesn't fit (or is
		# ANTIFLAG_GIVE-blocked) stays in the inventory. Everything goes out in
		# one network packet (see ENABLE_BULK_ITEM_SYSTEM).
		snapshot = multiSelectModule.PopDragSnapshot()
		remaining = [s for s in snapshot if s != (srcSlotType, usedSrcSlot)]

		itemList = [(player.SlotTypeToInvenType(srcSlotType), usedSrcSlot, usedDstSlot)]

		if remaining:
			freeDisplaySlots = []
			for pos in xrange(exchange.EXCHANGE_ITEM_MAX_NUM):
				if pos == usedDstSlot:
					continue
				if 0 == exchange.GetItemVnumFromSelf(pos):
					freeDisplaySlots.append(pos)

			added = 0
			for (slotType, slotIndex) in remaining:
				if added >= len(freeDisplaySlots):
					break

				invenType = player.SlotTypeToInvenType(slotType)
				itemID = player.GetItemIndex(invenType, slotIndex)
				if 0 == itemID:
					continue
				...
				itemList.append((invenType, slotIndex, freeDisplaySlots[added]))
				added += 1

			notAdded = len(remaining) - added
			if notAdded > 0:
				chat.AppendChat(chat.CHAT_TYPE_INFO, "%d item kimaradt a kereskedesbol." % notAdded)

		net.SendExchangeItemAddPacketBulk(itemList)
