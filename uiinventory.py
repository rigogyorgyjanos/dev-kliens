...
class InventoryWindow(ui.ScriptWindow):
	...
	def SelectEmptySlot(self, selectedSlotPos):
		...
		if mouseModule.mouseController.isAttached():
			...
			if player.SLOT_TYPE_INVENTORY == attachedSlotType or\
				...
				if multiSelectModule.dragSnapshot:
					self.__ProcessBulkMove(player.SLOT_TYPE_INVENTORY, attachedSlotPos, selectedSlotPos)
				else:
	...
	def SelectItemSlot(self, itemSlotIndex):
		...
		if mouseModule.mouseController.isAttached():
			...
			if app.SELL == curCursorNum:
			...
			elif app.IsPressed(app.DIK_LCONTROL) and app.IsPressed(app.DIK_LSHIFT):
				multiSelectModule.ToggleSlot(player.SLOT_TYPE_INVENTORY, itemSlotIndex)
				self.__RefreshSelectionVisual()
				...
				multiSelectModule.BeginDrag(player.SLOT_TYPE_INVENTORY, itemSlotIndex)
	...
	def __ProcessBulkMove(self, srcSlotType, usedSrcSlot, usedDstSlot):
		# The clicked item goes exactly where clicked; the rest of the selection
		# is auto-filled into the nearest empty slots AROUND that click (not the
		# first free slot on the page) and everything goes out in one network
		# packet (see ENABLE_BULK_ITEM_SYSTEM) instead of one packet per item.
		if uiPrivateShopBuilder.IsBuildingPrivateShop():
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.MOVE_ITEM_FAILURE_PRIVATE_SHOP)
			multiSelectModule.PopDragSnapshot()
			return

		snapshot = multiSelectModule.PopDragSnapshot()
		remaining = [s for s in snapshot if s != (srcSlotType, usedSrcSlot) and player.GetItemCount(s[1]) > 0]

		dstInvenType = player.SlotTypeToInvenType(srcSlotType)
		packetList = [(dstInvenType, usedSrcSlot, dstInvenType, usedDstSlot)]

		if remaining:
			# Search the WHOLE inventory (all pages), not just the page that was
			# clicked on - the distance sort still naturally prefers same-page
			# slots first (they're numerically closest to usedDstSlot), and only
			# spills onto other pages if the current one doesn't have enough room.
			totalSlotCount = player.INVENTORY_PAGE_SIZE * player.INVENTORY_PAGE_COUNT
			emptySlots = []
			for slotNumber in xrange(totalSlotCount):
				if slotNumber == usedDstSlot:
					continue
				if player.GetItemCount(slotNumber) == 0:
					emptySlots.append(slotNumber)

			emptySlots.sort(lambda a, b: cmp(abs(a - usedDstSlot), abs(b - usedDstSlot)))

			for i in xrange(min(len(remaining), len(emptySlots))):
				srcType, srcSlot = remaining[i]
				packetList.append((player.SlotTypeToInvenType(srcType), srcSlot, dstInvenType, emptySlots[i]))

			notMoved = len(remaining) - len(emptySlots)
			if notMoved > 0:
				chat.AppendChat(chat.CHAT_TYPE_INFO, "%d item nem fert el, kijelolve maradt." % notMoved)

		net.SendItemMovePacketBulk(packetList)
...
#			print "Belt Global Pos : ", self.wndBelt.GetGlobalPosition()
	...
	def __RefreshSelectionVisual(self):
		# Full recompute (not incremental) so this never has to know whether a slot's
		# current glow came from the pickup-glow feature or from a bulk selection.
		for i in xrange(self.wndItem.GetSlotCount()):
			slotNumber = self.__InventoryLocalSlotPosToGlobalSlotPos(i)
			isPickupGlow = app.BL_ENABLE_PICKUP_ITEM_EFFECT and (slotNumber in self.listHighlightedSlot)
			if multiSelectModule.IsSelected(player.SLOT_TYPE_INVENTORY, slotNumber) or isPickupGlow:
				self.wndItem.ActivateSlot(i)
			else:
				self.wndItem.DeactivateSlot(i)
...
class ExtendedInventoryWindow(ui.ScriptWindow):
	...
	def __RefreshSelectionVisual(self):
		currentSlotType = self.__GetCurrentSlotType()
		for i in xrange(self.wndItem.GetSlotCount()):
			slotNumber = self.__LocalSlotToGlobalSlot(i)
			isPickupGlow = app.BL_ENABLE_PICKUP_ITEM_EFFECT and (slotNumber in self.listHighlightedSlot)
			if multiSelectModule.IsSelected(currentSlotType, slotNumber) or isPickupGlow:
				self.wndItem.ActivateSlot(i)
			else:
				self.wndItem.DeactivateSlot(i)
	...
	def SelectEmptySlot(self, selectedSlotPos):
		...
		if mouseModule.mouseController.isAttached():
			...
			if player.SLOT_TYPE_INVENTORY == attachedSlotType or player.SLOT_TYPE_SKILL_BOOK_INVENTORY == attachedSlotType or player.SLOT_TYPE_UPGRADE_ITEMS_INVENTORY == attachedSlotType or player.SLOT_TYPE_STONE_INVENTORY == attachedSlotType or player.SLOT_TYPE_SANDIK_INVENTORY == attachedSlotType:
				...
				if multiSelectModule.dragSnapshot:
					self.__ProcessBulkMove(attachedSlotType, attachedSlotPos, selectedSlotPos)
				else:
					self.__SendMoveItemPacket(attachedSlotPos, selectedSlotPos, attachedCount)
	...
	def SelectItemSlot(self, itemSlotIndex):
		...
		if mouseModule.mouseController.isAttached():
			...
			if app.SELL == curCursorNum:
				...
				multiSelectModule.ToggleSlot(self.__GetCurrentSlotType(), itemSlotIndex)
				self.__RefreshSelectionVisual()
				...
				itemCount = player.GetItemCount(itemSlotIndex)
				currentSlotType = self.__GetCurrentSlotType()
				mouseModule.mouseController.AttachObject(self, currentSlotType, itemSlotIndex, selectedItemVNum, itemCount)
				multiSelectModule.BeginDrag(currentSlotType, itemSlotIndex)

				self.wndItem.SetUseMode(True)

				snd.PlaySound("sound/ui/pick.wav")

	def __GetCurrentSlotType(self):
		if self.inventoryType == 0:
			return player.SLOT_TYPE_SKILL_BOOK_INVENTORY
		elif self.inventoryType == 2:
			return player.SLOT_TYPE_STONE_INVENTORY
		elif self.inventoryType == 3:
			return player.SLOT_TYPE_SANDIK_INVENTORY
		else:
			return player.SLOT_TYPE_UPGRADE_ITEMS_INVENTORY
	...
	def __ProcessBulkMove(self, srcSlotType, usedSrcSlot, usedDstSlot):
		...
		if remaining:
			...
			# Search all 3 pages of this tab, not just the one that was clicked on -
			# the distance sort still naturally prefers same-page slots first.
			emptySlots = []
			for pageOffset in (0, 45, 90):
				for i in xrange(slotCount):
